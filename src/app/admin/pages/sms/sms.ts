...
export class SmsComponent implements OnInit {
  ...
  readonly statusOptions = APPOINTMENT_STATUSES;
  ...
  readonly form = this.fb.nonNullable.group({
    recipientSource: ['all' as SmsRecipientSource],
    statusFilter: ['Pending' as AppointmentStatus],
    manualNumbers: [''],
    ...
    this.adminApi
      ...
      .subscribe({
        next: (r) => {
          ...
          this.result.set(
            r.success
              ? `${r.recipientCount} kişiye gönderildi. Kod: ${r.resultCode}`
              : `Gönderim tamamlanamadı. Kod: ${r.resultCode}`,
          );
          this.loadLogs();
        },
        error: () => {
